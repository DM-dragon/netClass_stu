...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
  ...
  <script>
    $(function() {
		...
		$("#head-form").show();
		if(location.href=='http://localhost:8080/netClass-wwz/front_record.do'){
			$("#head-form").hide();
		}
    });
			...
			//判断是否需要签到
             $.ajax({
                ...
                success:function(data){
                    ...
                    if (nowDate-data.login_date <= 24*60*60*1000) {
                        $(".signBtn").html("已签到").unbind("click").addClass('gray').removeClass('hoverRed');     
                    } 
                }
            });
     ...
     function checkRePassword(){
          if ($("#modifyForm [name=rePassword]").val() != $("#modifyForm [name=newPassword]").val() ) {
              var $span = $("#modifyForm [name=rePassword]").parent().find("span");
                     ...
                     $span.css("color","red");
                     $("#modifyForm [name=rePassword]").empty();
                     $("#modifyForm [name=rePassword]").focus();
         }
         if ($("#modifyForm [name=rePassword]").val() == $("#modifyForm [name=newPassword]").val() ) {
              var $span = $("#modifyForm [name=rePassword]").parent().find("span");
                     $span.html("密码输入一致")
    ...
    <div class="modal fade" id="login_modal" tabindex="-1" role="dialog" aria-labelledby="myLoginLabel">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <form action="${pageContext.request.contextPath}/user/loginFront.do" class="form-horizontal" method="post">
                    <div class="modal-body">
                        <div class="form-group">
                            <label class="col-sm-3 control-label">登录名：</label>
                            <div class="col-sm-6">
